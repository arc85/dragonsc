...
---
# DRAGON for single-cell RNAseq clustering

DeteRminisitic Annealing Gaussian mixture mOdels for clusteriNg (DRAGON) is a clustering algorithm for single-cell RNAseq built upon the statistical framework of Gaussian mixture models and the concept of deterministic annealing. Analagous to the physical process of annealing, DRAGON uses a temperature parameter to put a constraint on the minimum entropy of the clustering solution. At each temperature, DRAGON performs maximum likelihood estimation to ascertain the paramaters of a Gaussian mixture model at that temperature. This allows us to define the "strongest" divisions in the data at higher temperatures, and to reveal more subtle differences as we gradually cool the temperature. This framework also allows us to split clusters in a statistically meaningful way as we reduce the temperature.
...
overall.clustering.soln <- dragon(pca.components=pcs.for.dragon,temp.decay.steps=annealing.steps,max.iterations=20,delta.log.likelihood=1e-6,max.clusters=6,save.intermediate.files=TRUE,num.cores=2,verbose=TRUE)
